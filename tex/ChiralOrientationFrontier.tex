...
\begin{document}
  ...
  \cong \mathrm{Sym}^2(V)$~\cite{Beau2026prs}.
    ...
    is exactly zero, and $\thmax\cdot q$ is $q$-invariant, with the same value at $q\in\{61,101\}$.
...
because $\varphi$ contains the antiunitary $\JPi$.
    ...
    For a real lift $U\in\mathrm{SL}(2,\mathbb{R})$ one has $\two\cong\twobar$ (the representation is self-conjugate), so
    the chiral distinction collapses and $\sigma_L = 0$, matching the chiral symmetry of the real cascade in
    which $\mu = 0$~\cite[Rem.~6.4]{Beau2026q14}.
...
inherited phase.
  ...
  \section{Frontier overlap and the conditional signal}
    ...
    On the nonzero-cocycle support the natural branch of the inherited phase, selected by the sign of its imaginary part
    at the radially outgoing representative, satisfies
    \begin{equation}
      \sigma_L(e) = \sgn\,\Im\,\zq^{\,\dac(e)} = \sgn \dac(e),
    \end{equation}
    since for a centred residue $r\in(-q/2,q/2)$ one has $\sgn\sin(2\pi r/q) = \sgn r$.
    This $\sigma_L$ is a function of the unoriented edge class, with the canonical radial cascade orientation fixing the
    representative, hence arrow-even; the conjugation that flips the area on $\varphi e$ makes it $\varphi$-odd.
...
\end{equation}
in this lift.
The bound $\thmax$ is thereby promoted to a nonzero signal under the named phase hypothesis.
    \end{theorem}
...
ADE-spin factor, then $\langle\sigma_L\,\sgn\dac\rangle_{\partial^+}$ could vanish by character orthogonality, yielding
a clean no-go of the same type as the arithmetic obstruction governing the ADE case selection of the generation
gate~\cite{Beau2026q14}.
